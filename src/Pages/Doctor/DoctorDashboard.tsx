    ...
    <DoctorLayout>
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-[#203a6d]">{d.name}</h1>
            <div className="text-sm text-gray-500">{d.role} • {d.today.date}</div>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <div className="text-sm text-gray-500">Messages</div>
              <div className="text-lg font-semibold">{d.messages}</div>
            </div>
            <div className="text-right">
              <div className="text-sm text-gray-500">Pending Labs</div>
              <div className="text-lg font-semibold">{d.pendingLabs}</div>
            </div>
          </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className="col-span-2 bg-white rounded-2xl shadow p-6 border border-[#e5e7eb]">
          ...
          <div className="bg-white rounded-2xl shadow p-6 border border-[#e5e7eb]">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#f8fbff] p-3 rounded">
                <div className="text-sm text-gray-500">Today</div>
                <div className="text-xl font-bold">{d.today.appointmentsCount}</div>
              </div>
              <div className="bg-[#fff7f6] p-3 rounded">
                <div className="text-sm text-gray-500">Checked In</div>
                <div className="text-xl font-bold">{d.today.checkedIn}</div>
              </div>
              <div className="bg-[#fffaf7] p-3 rounded">
                <div className="text-sm text-gray-500">Weekly Appts</div>
                <div className="text-xl font-bold">{d.weekly.appointments}</div>
              </div>
              <div className="bg-[#f4fff7] p-3 rounded">
                <div className="text-sm text-gray-500">Avg Wait (min)</div>
                <div className="text-xl font-bold">{d.weekly.avgWaitMin}</div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow p-6 border border-[#e5e7eb] mb-8">
          ...
          <ul className="space-y-2">
            {notifications.map((n: Notification) => {
              ...
              return (
                <li key={n.id} className={`flex items-start gap-3 p-3 rounded ${isRecent ? 'bg-[#f4f9ff]' : ''} hover:bg-[#f8fbff]`}>
                  <div className="w-36 text-xs text-gray-500 break-words">
